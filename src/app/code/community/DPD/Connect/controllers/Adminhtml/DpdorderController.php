 ...
 */
class DPD_Connect_Adminhtml_DpdorderController extends Mage_Adminhtml_Controller_Action
{
    /**
     ...
     * Fetches the label and puts it in a download response.
     */
    public function downloadDpdReturnLabelAction()
    {
        $returnLabelId = $this->getRequest()->getParam('label_id');
        $returnlabel = Mage::getModel('dpd/returnlabels')->load($returnLabelId);

        if ($returnlabel) {
            $pdf_Path = Mage::getBaseDir('media') . "/dpd/returnlabel/" . $returnlabel->getLabelPdfUrl();
            $pdf = Zend_Pdf::load($pdf_Path);
            $this->_prepareDownloadResponse($returnlabel->getLabelPdfUrl(), $pdf->render(), 'application/pdf');
        }
    }
