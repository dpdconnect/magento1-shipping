...
/**
 * Class DPD_Connect_Model_Carrier_Dpdparcelshops
 */
class DPD_Connect_Model_Carrier_Dpdparcelshops extends Mage_Shipping_Model_Carrier_Abstract
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if (!Mage::getStoreConfig('carriers/' . $this->_code . '/active') || Mage::app()->getStore()->isAdmin() || $quote->getData('is_multi_shipping')) {
            return false;
        }
       ...
       $tracking_numberExploded = explode('-', $tracking_number);
	    ...
	    if(substr($tracking_numberExploded[1], 0, 3) == "MPS" || substr($tracking_numberExploded[1], 0, 3) == "B2C"){
		    $tracking_status->addData(
			    array(
				    'status' => '<a target="_blank" href="' . "https://tracking.dpd.de/status/".$locale."/shipment/" . $tracking_number . '">' . Mage::helper('dpd')->__('Track this shipment') . '</a>'
			    )
		    );
	    }else{
		    $tracking_status->addData(
			    array(
				    'status' => '<a target="_blank" href="' . "https://tracking.dpd.de/status/".$locale."/parcel/" . $tracking_number. '">' . Mage::helper('dpd')->__('Track this shipment') . '</a>'
			    )
		    );
	    }
